...
class Login_Model extends Model
{
    ...
    public function editUserName() {
        ...
        if (!empty($_POST['user_email']) && !empty($_POST["user_password"])) {
            ...
            // [todo] replace with email regex
            //elseif (!empty($_POST['user_email']) && preg_match("/^(?=.{2,64}$)[a-zA-Z][a-zA-Z0-9]*(?: [a-zA-Z0-9]+)*$/", $_POST['user_email'])) {
            } elseif (!empty($_POST['user_email'])) {
    ...
    public function registerNewUser() {
        ...
        /*
        [hack] commented out captcha until it's a problem
        $captcha = new Captcha();
        ...
        if (empty($_POST['user_password_new'])) {
            ...
            $this->errors[] = FEEDBACK_EMAIL_DOES_NOT_FIT_PATTERN;
                ...
                if ($count == 1) {
                    ...
                    //$query_new_user_insert = $this->db_connection->query("INSERT INTO users (user_password_hash, user_email, user_activation_hash) VALUES('".$this->user_password_hash."', '".$this->user_email."', '".$this->user_activation_hash."');");
    ...
    public function setPasswordResetDatabaseToken() {
        ...
        if (empty($_POST['user_email'])) {
            ...
            $this->errors[] = "Empty email address";
            ...
            $sth = $this->db->prepare("SELECT user_id, user_email FROM users WHERE user_email = :user_email ;");
            $sth->execute(array(':user_email' => $this->user_email));                    
    ...
    public function sendPasswordResetMail() {
        ...
        $link = EMAIL_PASSWORDRESET_URL.'/'.urlencode($this->user_email).'/'.urlencode($this->user_password_reset_hash);
     ...
     * 
     */
    public function verifypasswordrequest($user_email, $verification_code) {
